      ...
      {data.map((User: User, index: number) => (
        <View key={index} style={styles.user}>
          <View style={styles.row}>
            ...
            <Text>{User.auctionTitle}</Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.label}>Category:</Text>
            <Text>{User.auctionCategory}</Text>
          </View>
          <View style={styles.row}>
            <Text style={styles.label}>Bid Price:</Text>
            <Text>{User.bidPrice}</Text>
          </View>
          <View style={styles.row}>
      ...
      {showAlert && (
        <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-50 mt-32 flex justify-center items-center">
          <div className="bg-white p-10 rounded-lg shadow-lg">
            <p className="text-xl font-semibold ml-14 mb-4">Successfull</p>
            <p>Tarcking Detials Deleted</p>
            <button
              onClick={() => setShowAlert(false)}
              className="mt-4 bg-brownDark hover:bg-brownMedium text-white px-4 py-2 ml-20 rounded-xl  focus:outline-none focus:ring focus:border-blue-300"
            >
              Close
            </button>
          </div>
