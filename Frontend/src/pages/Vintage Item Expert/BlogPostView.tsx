...
const BlogPostView = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3000/vintageexpert/blog/getAll`,
        {}
      );
  ...
  const handleSearchSubmit = async (event: { preventDefault: () => void }) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/vintageexpert/blog/search?query=${searchQuery}`
      );
    ...
    <div className="flex">
      ...
      <div className="ml-4">
        ...
        <div className="flex justify-center mb-4">
          {" "}
          {/* Add flex and justify-center to center its children horizontally */}
        ...
        <div className="flex justify-start mb-4">
          <Link
            to="/blog/Post"
            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full inline-block mb-4"
          >
            +
          </Link>
        </div>
        ...
        {renderPosts.length > 0 ? (
          renderPosts.map((post) => (
            ...
            >
              <h3 className="text-xl font-bold font-serif text-amber-700  mb-2">
