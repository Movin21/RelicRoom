...
import { PDFDownloadLink, PDFViewer, Document, Page, View, Text, StyleSheet,Font, Image } from '@react-pdf/renderer';
  ...
  const MyDocument: React.FC<{ data: Bid[] }> = ({ data }) => (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.subtitle}>Bidding Details</Text>
         
          {data.map((bid) => (
            <View key={bidder._id}>
              <Text style={styles.description}>Auction Title: {bid.auction && bid.auction.auctionTitle}</Text>
              <Text style={styles.description}>Category: {bid.auction && bid.auction.auctionCategory}</Text>
              <Text style={styles.description}>Bidded Price: {bid.bidPrice.toString()}</Text>
              <Text style={styles.description}>Auctioneer: {bid.auctioneer && bid.auctioneer.companyName}</Text>
  ...
  return (
     <div className="flex min-h-screen flex-col bg-muted/40">
          ...
          My Bids
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="h-8 gap-1">
                  <span className="text-brownDark font-akshar sr-only sm:not-sr-only sm:whitespace-nowrap">
                    Sort by 
                  </span>
                  <svg width="12" height="12" viewBox="0 0 39 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M37.05 5.29412H1.95C1.43283 5.29412 0.936838 5.01523 0.571142 4.51881C0.205446 4.02239 0 3.3491 0 2.64706C0 1.94502 0.205446 1.27173 0.571142 0.775305C0.936838 0.278885 1.43283 0 1.95 0H37.05C37.5672 0 38.0632 0.278885 38.4289 0.775305C38.7946 1.27173 39 1.94502 39 2.64706C39 3.3491 38.7946 4.02239 38.4289 4.51881C38.0632 5.01523 37.5672 5.29412 37.05 5.29412ZM30.55 17.6471H8.45C7.93283 17.6471 7.43684 17.3682 7.07114 16.8718C6.70545 16.3753 6.5 15.702 6.5 15C6.5 14.298 6.70545 13.6247 7.07114 13.1282C7.43684 12.6318 7.93283 12.3529 8.45 12.3529H30.55C31.0672 12.3529 31.5632 12.6318 31.9289 13.1282C32.2946 13.6247 32.5 14.298 32.5 15C32.5 15.702 32.2946 16.3753 31.9289 16.8718C31.5632 17.3682 31.0672 17.6471 30.55 17.6471ZM22.75 30H16.25C15.7328 30 15.2368 29.7211 14.8711 29.2247C14.5054 28.7283 14.3 28.055 14.3 27.3529C14.3 26.6509 14.5054 25.9776 14.8711 25.4812C15.2368 24.9848 15.7328 24.7059 16.25 24.7059H22.75C23.2672 24.7059 23.7632 24.9848 24.1289 25.4812C24.4946 25.9776 24.7 26.6509 24.7 27.3529C24.7 28.055 24.4946 28.7283 24.1289 29.2247C23.7632 29.7211 23.2672 30 22.75 30Z" fill="#302300"/></svg>
                </Button>
              </DropdownMenuTrigger>

              <DropdownMenuContent align="end">
                <DropdownMenuLabel className=" text-brownDark font-akshar">Category</DropdownMenuLabel>
                <DropdownMenuCheckboxItem className=" text-brownDark font-akshar" onClick={() => handleFilter('Clothing')}  >
                  Clothing
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem className=" text-brownDark font-akshar" onClick={() => handleFilter('Furniture')}>
                Furniture
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem className=" text-brownDark font-akshar" onClick={() => handleFilter('Art')}>
                Art
              </DropdownMenuCheckboxItem>
              <DropdownMenuCheckboxItem className=" text-brownDark font-akshar" onClick={() => handleFilter('Jewelry')}>
                Jewelry
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <div className="flex justify-between,center ml-8 gap-6">
        <Card className="flex flex-col items-center gap-2 px-2 py-5 bg-white shadow-2xl h-96 w-80 mb-52 mt-0 ">
          <ul className="flex flex-col gap-4 mt-4">
            ...
            <li>
                  ...
                  <li>
                    <div onClick={handleWishlistlick} className="flex items-center mb-2 hover:scale-110 transition duration-300 ease-in-out">
                    ...
                    {filteredBids.map((bid: Bid, index: number) => (
                      <TableRow key={index} className="hover:bg-footertxt  hover:scale-105 transition duration-300 ease-in-out">
                        <TableCell className="hidden sm:table-cell font-akshar  ">
                          {bid.auction && bid.auction.auctionImages.length > 0 &&(
                            <img
                              src={bid.auction.auctionImages[0]}
                              alt={`Preview Image`}
