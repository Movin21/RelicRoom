...
const VintageexpertProfile = () => {
  ...
  const fetchPost = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3000/vintageexpert/get/${id}`
      );
  ...
  const deletePost = async () => {
    try {
      await axios.delete(
        `http://localhost:3000/vintageexpert/delete/${post._id}`
      );
        ...
        <ul className="flex flex-col gap-2">
          <li>
            <div className="flex items-center mb-2">
              <span className="text-brownDark font-akshar text-2xl font-bold  ml-2">
                Vintage Expert Dashboard
              </span>
            </div>
          </li>
          <li>
            <Link to="/vintageexpert/Profile/:id">
              <div className="flex items-center mb-2">
                <span className=" text-black font-akshar text-1xl font-semibold  ml-2">
                  {" "}
                  Profile Details
                </span>
              </div>
          ...
          <Link to="/vintageexpert/Login">
            <div className="flex items-center mb-2">
              <span className="text-brownDark font-akshar text-1xl font-semibold  ml-2">
                Log Out
              </span>
            </div>
          </Link>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {post ? (
          <div className="flex items-center justify-center">
            <div
              className="w-full md:w-96 p-6 mt-0 mb-10 shadow-2xl"
              style={{ width: "700px" }}
            >
              ...
              <div className="flex justify-center items-start space-x-20 mt-20">
                <Link
                  to={`/vintageexpert/Update/${post._id}`}
                  className="w-40 text-white bg-primary hover:bg-secondary ease-in-out hover:text-white font-bold py-2 px-4 rounded transition duration-700"
                >
                  Update Profile
                </Link>
                <button
                  onClick={deletePost}
                  className="bg-red-800 hover:bg-red-800 text-white font-bold py-2 px-4 rounded transition duration-300"
                >
                  Delete Profile
                </button>
                {deletionSuccess && (
