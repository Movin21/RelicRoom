...
function FeedbackManage() {
  ...
  const columnsFeedback = [
    {
      name: "UserID",
      selector: (_row: Feedback | Suggestion | Complaints, index: any) =>
        index + 1,
    },
    {
      name: "Name",
    ...
    },
    {
      ...
      cell: (row: Feedback | Suggestion | Complaints) => {
        let label;
        if ("IsFeedback" in row)
          label = <Badge className="bg-amber-700">Feedback</Badge>;
        else if ("IsSuggestion" in row)
          label = <Badge className="bg-orange-950">Suggestion</Badge>;
        else if ("IsComplaint" in row)
          label = <Badge className="bg-amber-900">Complaint</Badge>;
        return label;
      },
    },
    {
      ...
      cell: (
        ...
        handleRowExpand: (id: string) => void
      ) => (
        <div
    ...
    <div className="max-w-screen-lg px-4 mx-auto">
      <PDFDownloadLink document={<PdfFile />} fileName="PdfFile">
        {({ loading }) =>
          loading ? (
            ...
            <Button className="btnfeed">Details Download</Button>
          )
        }
      </PDFDownloadLink>
      ...
      <Tabs defaultValue="feedbackManage" className="w-full">
        <TabsList className="flex justify-center text-2xl font-bold h-500 item-center w-500 font-akshar text-yellow-950">
          <TabsTrigger value="feedbackManage" className="text-lg">
            Feedback Manage
          </TabsTrigger>
          <TabsTrigger value="faqManage" className="text-lg">
            <Link to="/FAQManage">FAQ Manage</Link>
          </TabsTrigger>
        </TabsList>
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <div className="w-full" ref={ComponentsRef}>
            <DataTable columns={columnsFeedback} data={filteredData} />
          </div>
        </div>
      </Tabs>
    </div>
